...
def get_id_input(id_supported_sites):
    tmp = input("site: ")
    if tmp in id_supported_sites:
        num = get_int_input("id: ")
        return num, tmp
    else:
...
def make_folder(name):
    if not os.path.exists(name):
        ...
        if not os.path.isdir(name):
            sys.exit("%s isn't a directory" % name)
...
def download_image(link):
    ...
    while True:
        try:
            ...
            data = r.content
            now = len(data)
            if ('content-length' not in r.headers or int(
                    r.headers['content-length']) == now) and now != 0:
                return data
            elif now == last:
                times += 1
                if times > 5:
                ...
                last = now
                print("error, file download size is %d while content-length "
                      "say %s"%(len(data),r.headers['content-length']))
    ...
    while True:
        try:
            ...
            if "</html>" in data.lower():
                ...
                print("error, the page doesn't have `</html>` in it, "
                      "possible fault in download")
